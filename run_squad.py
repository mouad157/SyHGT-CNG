    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if len(graph_data)==0:
            ...
            for i, feature_index in enumerate(feature_indices):
                ...
                start_logits,end_logits = torch.zeros(len(eval_feature.tokens)).tolist() , torch.zeros(len(eval_feature.tokens)).tolist()
        ...
        try:
            if not example.is_impossible:
                ...
                for answer_true in answers:
                    if (answer in answer_true["text"]  or answer_true["text"] in answer) and answer !="" :
                        print("=============================================================")
                        print(example.question_text, "well done")
                        print(example.context_text)
                        print("our answer : " ,answer, "||expected answer: ", answer_true["text"])
                        good_answers += 1
                        found = True
                        break
                if not found:
...
def main():
    ...
    model = GraphQA.from_pretrained(
        ...
        config=config,
        cache_dir=args.cache_dir if args.cache_dir else None,ignore_mismatched_sizes=True
    )
    ...
    # Training
    if args.do_train:
        train_dataset,_, features = load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=True)
        train_graph = load_graph(args, evaluate=False)
        # TODO(mingzhe): Merge these two parts
        global_step, tr_loss = train(args, train_dataset, train_graph,features, model, tokenizer)
